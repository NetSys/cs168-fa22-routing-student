...
def test_is_locally_k_edge_connected_exceptions():
    assert_raises(nx.NetworkXNotImplemented, is_k_edge_connected, nx.DiGraph(), k=0)
    assert_raises(nx.NetworkXNotImplemented, is_k_edge_connected, nx.MultiGraph(), k=0)
    assert_raises(ValueError, is_k_edge_connected, nx.Graph(), k=0)
...
def test_is_k_edge_connected_exceptions():
    ...
    assert_raises(ValueError, is_locally_k_edge_connected, nx.Graph(), 1, 2, k=0)
...
def test_unfeasible():
    ...
    assert_raises(nx.NetworkXUnfeasible, list, k_edge_augmentation(G, k=1, avail=[]))

    assert_raises(nx.NetworkXUnfeasible, list, k_edge_augmentation(G, k=2, avail=[]))
...
def _assert_solution_properties(G, aug_edges, avail_dict=None):
    ...
    if avail_dict is not None:
        assert_true(
            all(e in avail_dict for e in aug_edges),
    ...
    assert_true(len(aug_edges) == len(unique_aug), "edges should be unique")

    assert_false(any(u == v for u, v in unique_aug), "should be no self-edges")

    assert_false(
        any(G.has_edge(u, v) for u, v in unique_aug),
...
def _augment_and_check(
    ...
    try:
        ...
        try:
            ...
            assert_false(isinstance(generator, list), "should always return an iter")
            aug_edges = []
            ...
            assert_equal(
                len(aug_edges), 0, "should not generate anything if unfeasible"
            )
        ...
        assert_greater_equal(
            info["aug_k"], orig_k, ("augmenting should never reduce connectivity")
        )
        ...
        if aug_edges1 is not None:
            ...
            if k == 2:
                ...
                if orig_k == 0:
                    ...
                    assert_less_equal(info2["total_weight"], info1["total_weight"] * 3)
                else:
                    # the approximation ratio is 2 if G is was connected
                    assert_less_equal(info2["total_weight"], info1["total_weight"] * 2)
