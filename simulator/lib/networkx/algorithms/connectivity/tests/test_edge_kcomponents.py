...
def _check_edge_connectivity(G):
    ...
    for k in it.count(1):
        ...
        assert_equal(alt_subgraph_ccs, ccs_subgraph, "alt subgraph method failed")
...
def test_local_subgraph_difference_directed():
    ...
    assert_equal(fset(nx.k_edge_components(G, k=1)), fset(nx.k_edge_subgraphs(G, k=1)))
    ...
    assert_not_equal(
        fset(nx.k_edge_components(G, k=2)), fset(nx.k_edge_subgraphs(G, k=2))
    )

    assert_equal(fset(nx.k_edge_components(G, k=3)), fset(nx.k_edge_subgraphs(G, k=3)))
...
def test_triangles():
    ...
    assert_equal(fset(nx.k_edge_components(G, k=2)), fset(nx.k_edge_subgraphs(G, k=2)))
...
def test_five_clique():
    ...
    assert_not_equal(
        fset(nx.k_edge_components(G, k=4)), fset(nx.k_edge_subgraphs(G, k=4))
    )

    # For k=5 they are not the same
    assert_not_equal(
        fset(nx.k_edge_components(G, k=5)), fset(nx.k_edge_subgraphs(G, k=5))
    )

    # For k=6 they are the same
    assert_equal(fset(nx.k_edge_components(G, k=6)), fset(nx.k_edge_subgraphs(G, k=6)))
    _check_edge_connectivity(G)
