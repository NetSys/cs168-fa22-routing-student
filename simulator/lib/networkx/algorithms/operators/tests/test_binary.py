...
def test_symmetric_difference_multigraph():
    ...
    assert_equal(
        sorted(sorted(e) for e in gh.edges(keys=True)),
        [[0, 1, 1], [0, 1, 2], [0, 1, 3]],
    )
...
def test_union_and_compose():
    ...
    assert_equal(
        sorted(G2.nodes()), ["1", "2", "3", "4", "copy1", "copy2", "copy3", "copy4"]
    )
...
def test_union_multigraph():
    ...
    assert_equal(
        set(GH.edges(keys=True)), set(G.edges(keys=True)) | set(H.edges(keys=True))
    )
