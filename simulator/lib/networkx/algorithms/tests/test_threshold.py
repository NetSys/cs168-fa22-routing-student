...
class TestGeneratorThreshold:
    ...
    def test_make_compact(self):
        ...
        assert_raises(TypeError, nxt.make_compact, [3.0, 1.0, 2.0])

    def test_uncompact(self):
        assert_equal(nxt.uncompact([3, 1, 2]), ["d", "d", "d", "i", "d", "d"])
        assert_equal(nxt.uncompact(["d", "d", "i", "d"]), ["d", "d", "i", "d"])
        assert_equal(
            nxt.uncompact(nxt.uncompact([(1, "d"), (2, "d"), (3, "i"), (0, "d")])),
            nxt.uncompact([(1, "d"), (2, "d"), (3, "i"), (0, "d")]),
        )
        assert_raises(TypeError, nxt.uncompact, [3.0, 1.0, 2.0])

    def test_creation_sequence_to_weights(self):
        assert_equal(
            nxt.creation_sequence_to_weights([3, 1, 2]),
            [0.5, 0.5, 0.5, 0.25, 0.75, 0.75],
        )
        assert_raises(TypeError, nxt.creation_sequence_to_weights, [3.0, 1.0, 2.0])
    ...
    def test_weights_to_creation_sequence(self):
        ...
        assert_equal(
            nxt.weights_to_creation_sequence(deg, with_labels=True),
            [(3, "d"), (1, "d"), (2, "d"), (0, "d")],
        )
    ...
    def test_shortest_path(self):
        ...
        for n, m in [(3, 0), (0, 3), (0, 2), (0, 1), (1, 3), (3, 1), (1, 2), (2, 3)]:
            assert_equal(nxt.shortest_path(cs1, n, m), nx.shortest_path(G, n, m))
        ...
        assert_raises(TypeError, nxt.shortest_path, [3.0, 1.0, 2.0], 1, 2)
    ...
    def test_shortest_path_length(self):
        ...
        assert_equal(
            nxt.shortest_path_length(["d", "d", "d", "i", "d", "d"], 1),
            [1, 0, 1, 2, 1, 1],
        )
        assert_equal(
            nxt.shortest_path_length(("d", "d", "d", "i", "d", "d"), 1),
        ...
        assert_raises(TypeError, nxt.shortest_path, [3.0, 1.0, 2.0], 1)

    def random_threshold_sequence(self):
        assert_equal(len(nxt.random_threshold_sequence(10, 0.5)), 10)
        assert_equal(
            nxt.random_threshold_sequence(10, 0.5, seed=42),
            ["d", "i", "d", "d", "d", "i", "i", "i", "d", "d"],
        )
    ...
    def test_weights_thresholds(self):
        ...
        assert_equal(wseq, [s * 0.1 for s in [5, 5, 4, 6, 3, 3, 3, 7, 2, 8, 1, 9, 0]])
    ...
    def test_fast_versions_properties_threshold_graphs(self):
        ...
        assert_equal(sorted(nxt.degree_sequence(cs)), sorted(d for n, d in G.degree()))
