    ...
    def test_unorderable_nodes(self):
        ...
        class Unorderable(object):
            def __lt__(self, other):
                error_msg = "< not supported between instances of " "{} and {}".format(
...
class TestDAG:
    ...
    def test_lexicographical_topological_sort(self):
        ...
        assert_equal(list(nx.lexicographical_topological_sort(G)), [1, 2, 3, 4, 5, 6])
        assert_equal(
            list(nx.lexicographical_topological_sort(G, key=lambda x: x)),
            [1, 2, 3, 4, 5, 6],
        )
        assert_equal(
            list(nx.lexicographical_topological_sort(G, key=lambda x: -x)),
...
def test_is_aperiodic_raise():
    G = nx.Graph()
    assert_raises(nx.NetworkXError, nx.is_aperiodic, G)
