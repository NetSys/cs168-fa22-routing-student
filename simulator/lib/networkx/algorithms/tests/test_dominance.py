...
class TestImmediateDominators(object):
    ...
    def test_path(self):
        ...
        assert_equal(
            nx.immediate_dominators(G, 0), {i: max(i - 1, 0) for i in range(n)}
        )
    ...
    def test_irreducible1(self):
        ...
        G = nx.DiGraph(edges)
        assert_equal(nx.immediate_dominators(G, 5), {i: 5 for i in range(1, 6)})
    ...
    def test_irreducible2(self):
        ...
        G = nx.DiGraph(edges)
        assert_equal(nx.immediate_dominators(G, 6), {i: 6 for i in range(1, 7)})
    ...
    def test_domrel_png(self):
        ...
        G = nx.DiGraph(edges)
        assert_equal(
            nx.immediate_dominators(G, 1), {1: 1, 2: 1, 3: 2, 4: 2, 5: 2, 6: 2}
        )
        # Test postdominance.
        with nx.utils.reversed(G):
            assert_equal(
                nx.immediate_dominators(G, 6), {1: 2, 2: 6, 3: 5, 4: 5, 5: 2, 6: 6}
            )
...
class TestDominanceFrontiers(object):
    ...
    def test_path(self):
        n = 5
        G = nx.path_graph(n, create_using=nx.DiGraph())
        assert_equal(nx.dominance_frontiers(G, 0), {i: set() for i in range(n)})
    ...
    def test_unreachable(self):
        ...
        G = nx.path_graph(n, create_using=nx.DiGraph())
        assert_equal(
            nx.dominance_frontiers(G, n // 2), {i: set() for i in range(n // 2, n)}
        )
    ...
    def test_irreducible1(self):
        ...
        G = nx.DiGraph(edges)
        assert_equal(
            {u: df for u, df in nx.dominance_frontiers(G, 5).items()},
            {1: set([2]), 2: set([1]), 3: set([2]), 4: set([1]), 5: set()},
        )
    ...
    def test_domrel_png(self):
        ...
        assert_equal(
            ...
            {1: set([]), 2: set([2]), 3: set([5]), 4: set([5]), 5: set([2]), 6: set()},
        )
        # Test postdominance.
        with nx.utils.reversed(G):
            assert_equal(
