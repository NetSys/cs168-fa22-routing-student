...
def validate_flows(G, s, t, flowDict, solnValue, capacity, flow_func):
    ...
    for u in G:
        assert_equal(set(G[u]), set(flowDict[u]), msg=msg.format(flow_func.__name__))
...
def validate_cuts(G, s, t, solnValue, partition, capacity, flow_func):
    assert_true(all(n in G for n in partition[0]), msg=msg.format(flow_func.__name__))
    assert_true(all(n in G for n in partition[1]), msg=msg.format(flow_func.__name__))
    cutset = compute_cutset(G, partition)
    assert_true(
        all(G.has_edge(u, v) for (u, v) in cutset), msg=msg.format(flow_func.__name__)
    )
    assert_equal(
        solnValue,
        sum(G[u][v][capacity] for (u, v) in cutset),
    ...
    if not G.is_directed():
        ...
        assert_false(nx.is_strongly_connected(H), msg=msg.format(flow_func.__name__))
...
class TestMaxflowMinCutCommon:
    ...
    def test_digraph_infcap_path(self):
        ...
        for flow_func in all_funcs:
            assert_raises(nx.NetworkXUnbounded, flow_func, G, "s", "t")
    ...
    def test_flow_func_not_callable(self):
        ...
        for flow_func in interface_funcs:
            for element in elements:
                assert_raises(nx.NetworkXError, flow_func, G, 0, 1, flow_func=element)
                assert_raises(nx.NetworkXError, flow_func, G, 0, 1, flow_func=element)
        ...
        for interface_func in interface_funcs:
            for flow_func in flow_funcs:
                ...
                assert_equal(
                    fv,
                    result,
                    msg=msgi.format(flow_func.__name__, interface_func.__name__),
                )
...
def test_preflow_push_global_relabel_freq():
    ...
    assert_raises(nx.NetworkXError, preflow_push, G, 1, 2, global_relabel_freq=-1)
