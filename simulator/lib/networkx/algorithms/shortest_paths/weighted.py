...
def bellman_ford_predecessor_and_distance(
    ...
    dist = _bellman_ford(
        G, [source], weight, pred=pred, dist=dist, cutoff=cutoff, target=target
    )
    return (pred, dist)


def _bellman_ford(
    G, source, weight, pred=None, paths=None, dist=None, cutoff=None, target=None
):
...
def single_source_bellman_ford(G, source, target=None, cutoff=None, weight="weight"):
    ...
    dist = _bellman_ford(G, [source], weight, paths=paths, cutoff=cutoff, target=target)
    if target is None:
        return (dist, paths)
    try:
        return (dist[target], paths[target])
