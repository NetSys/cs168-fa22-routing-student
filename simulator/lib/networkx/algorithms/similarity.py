...
def optimize_edit_paths(
    ...
    def make_CostMatrix(C, m, n):
        ...
        subst_ind = list(
            k
            for k, i, j in zip(range(len(lsa_row_ind)), lsa_row_ind, lsa_col_ind)
            if i < m and j < n
        )
        dummy_ind = list(
            k
    ...
    def match_edges(u, v, pending_g, pending_h, Ce, matched_uv=[]):
        ...
        g_ind = list(
            i
            for i in range(M)
            if any(pending_g[i] in ((p, u), (u, p), (u, u)) for p, q in matched_uv)
        )
        h_ind = list(
            j
            for j in range(N)
            if any(pending_h[j] in ((q, v), (v, q), (v, v)) for p, q in matched_uv)
        )
        m = len(g_ind)
        n = len(h_ind)
        ...
        if not max(len(pending_u), len(pending_v)):
            ...
            for ij, Cv_ij, xy, Ce_xy, edit_cost in edit_ops:
                ...
                for x, y in xy:
                    matched_gh.append(
                        (
                            pending_g[x] if x < len(pending_g) else None,
