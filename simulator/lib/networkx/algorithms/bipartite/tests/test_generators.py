...
class TestGeneratorsBipartite:
    ...
    def test_configuration_model(self):
        aseq = [3, 3, 3, 3]
        bseq = [2, 2, 2, 2, 2]
        assert_raises(networkx.exception.NetworkXError, configuration_model, aseq, bseq)

        aseq = [3, 3, 3, 3]
        bseq = [2, 2, 2, 2, 2, 2]
        G = configuration_model(aseq, bseq)
        assert_equal(sorted(d for n, d in G.degree()), [2, 2, 2, 2, 2, 2, 3, 3, 3, 3])
        ...
        bseq = [3, 3, 3]
        G = configuration_model(aseq, bseq)
        assert_equal(sorted(d for n, d in G.degree()), [1, 1, 1, 2, 2, 2, 3, 3, 3])
    ...
    def test_havel_hakimi_graph(self):
        aseq = [3, 3, 3, 3]
        bseq = [2, 2, 2, 2, 2]
        assert_raises(networkx.exception.NetworkXError, havel_hakimi_graph, aseq, bseq)
    ...
    def test_reverse_havel_hakimi_graph(self):
        ...
        bseq = [2, 2, 2, 2, 2]
        assert_raises(
            networkx.exception.NetworkXError, reverse_havel_hakimi_graph, aseq, bseq
        )
    ...
    def test_alternating_havel_hakimi_graph(self):
        ...
        bseq = [2, 2, 2, 2, 2]
        assert_raises(
            networkx.exception.NetworkXError, alternating_havel_hakimi_graph, aseq, bseq
        )
